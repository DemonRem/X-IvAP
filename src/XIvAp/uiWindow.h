...
#define CURSOR_INTERVAL	0.5f// 0.5f is origin
...
public:
	...
	void keyHome();
	void keyEnd();
	void keyUpArrow(XPLMKeyFlags inFlags);
	void keyDownArrow(XPLMKeyFlags inFlags);
	void keyLeftArrow(XPLMKeyFlags inFlag);
	void keyRightArrow(XPLMKeyFlags inFlags);
	...
	void setEditLine(const string& text) { mEditLine = text; };
	string getEditLine() const { return mEditLine; };
...
private:
	...
	// cmd line history
	size_t mMaxCmdLineHistorySize;
	int mCurrentCmdLineHistoryPos;
	std::vector<std::string>mCmdLineHistory;
	string mSavedEditLine;

	string mEditLine;	// the text line as it is being edited by the user
	bool mIsCursorVisible;		// blinking cursor
	float mCursorTime;	// last time cursor changed status
	size_t mCursorPos;
