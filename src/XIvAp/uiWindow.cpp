...
void UiWindow::addMessage(float *color, const pt::string& text, bool logToConsole, bool logToLogfile)
{
	...
	if(pExternalInterfaceServer)
        {
          std::string externalInterfaceMessage("XIVAPEI_SV_UIMESSAGE|");
          externalInterfaceMessage.append(text.stl_str());

          pExternalInterfaceServer->BroadcastMessage(externalInterfaceMessage);
        }
